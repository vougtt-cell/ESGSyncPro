...
async function ensureSampleSurveyCompanies() {
    ...
    for (const b of benchmarks) {
        const existing = await query(
            'SELECT id FROM survey_comparison_companies WHERE company_name = $1 AND industry = $2',
    ...
    if (addedCount > 0) {
        console.log(`Added ${addedCount} sample survey companies`);
    }
}
...
async function ensureSampleArticles() {
    const articles = [
        ...
        },
        {
            ...
            content_pl: 'Zrównoważone praktyki biznesowe ewoluują szybko. Od modeli gospodarki o obiegu zamkniętym po operacje neutralne pod kątem emisji dwutlenku węgla, firmy przyjmują innowacyjne podejście do zmniejszenia wpływu na środowisko przy zachowaniu rentowności.',
    ...
    if (addedCount > 0) {
        console.log(`Added ${addedCount} sample articles`);
    }
}
...
async function ensureSampleCompanyBenchmarks() {
    ...
    for (const b of benchmarks) {
        const existing = await query(
            'SELECT id FROM company_benchmarks WHERE company_name = $1 AND industry = $2',
            [b.company_name, b.industry]
        );
        if (existing.rows.length === 0) {
            await query(
    ...
    if (addedCount > 0) {
        console.log(`Added ${addedCount} sample company benchmarks`);
    }
}
...
async function ensureSampleCompanies() {
    ...
    for (const c of companies) {
        const existing = await query(
            'SELECT id FROM companies WHERE name = $1 AND industry = $2',
            [c.name, c.industry]
        );
        if (existing.rows.length === 0) {
            await query(
    ...
    if (addedCount > 0) {
        console.log(`Added ${addedCount} sample companies`);
    }
}
